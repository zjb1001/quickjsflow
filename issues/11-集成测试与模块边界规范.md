...
- 测试框架：`test/test_framework.h`（C 宏断言、统计）
- 集成测试：`test/test_integration.c`（Lexer+Parser 端到端）
- Round-trip：`test/test_roundtrip.c`（parse→print→parse 验证）
- 构建与运行：`make test` 编译并执行所有集成测试
- 当前状态：15 个测试通过，框架已就位，等待后续模块（Scope, Edit, Codegen）集成
